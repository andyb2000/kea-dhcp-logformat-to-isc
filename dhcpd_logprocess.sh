...
process_log_entry() {
    ...
    if [[ "$entry" == *"DHCP4_REQUEST"* ]]; then
        ...
        if [[ -n "$mac" && -n "$ip" ]]; then
            ...
            # ISC dhcpd log style: DHCPREQUEST for 192.168.55.224 from 74:83:c2:c0:fe:53 (hostname)
    ...
    if [[ "$entry" == *"DHCP4_LEASE_ALLOC"* ]]; then
        ...
        if [[ -n "$mac" && -n "$ip" ]]; then
            # ISC dhcpd log style: DHCPACK on 192.168.55.224 to 74:83:c2:c0:fe:53 (hostname) requested 192.168.55.224 lease 6000s
